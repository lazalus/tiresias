...
const defaultLandingUrl = optionalEnv('NAVER_ADS_DEFAULT_LANDING_URL', 'https://tiresiasview.com')
const customerId = sanitizeCustomerId(optionalEnv('NAVER_ADS_CUSTOMER_ID'))
...
const plans = resolveBlueprintPaths(args).map((blueprintPath) => {
  ...
  return {
    ...
    deviceMix: blueprint.deviceMix,
    adGroups: blueprint.adGroups.map((adGroup) => ({
      name: adGroup.name,
      keywordCount: adGroup.keywords.length,
      headlineCount: adGroup.ads.headlines.length,
      descriptionCount: adGroup.ads.descriptions.length,
    })),
  }
})
