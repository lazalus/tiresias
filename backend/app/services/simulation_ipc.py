...
파일 시스템을 통해 간단한 명령/응답 패턴 구현:
1. Flask는 commands/ 디렉토리에 명령을 작성
2. 시뮬레이션 스크립트는 명령 디렉토리를 폴링하고, 명령을 실행하며 responses/ 디렉토리에 응답을 작성
3. Flask는 응답 디렉토리를 폴링하여 결과를 가져옴
"""
...
logger = get_logger('tiresias.simulation_ipc')
...
class SimulationIPCClient:
    """
    IPC 클라이언트 시뮬레이션 (Flask 측 사용)
    
    시뮬레이션 프로세스에 명령을 보내고 응답을 기다리는 데 사용
    """
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: 지정 플랫폼 (선택 사항)
                ...
                - None: 이중 플랫폼 시뮬레이션 시 두 플랫폼을 동시에 인터뷰하고, 단일 플랫폼 시뮬레이션 시 해당 플랫폼을 인터뷰
            timeout: 타임아웃 시간
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: 기본 플랫폼 (선택 사항, 각 인터뷰 항목의 platform에 의해 덮어쓰여질 수 있음)
                - "twitter": 기본적으로 Twitter 플랫폼만 인터뷰
                - "reddit": 기본적으로 Reddit 플랫폼만 인터뷰
                - None: 이중 플랫폼 시뮬레이션 시 각 Agent가 두 플랫폼을 동시에 인터뷰
            timeout: 타임아웃 시간
...
class SimulationIPCServer:
    """
    IPC 서버 시뮬레이션 (시뮬레이션 스크립트 측 사용)
    
    명령 디렉토리를 폴링하고, 명령을 실행하며 응답을 반환
    """
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        """
        명령 디렉토리를 폴링하고, 첫 번째 처리 대기 중인 명령을 반환
