<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">추론 기록</span>
      <div class="section-line"></div>
    </div>
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- 카드 제목 (시뮬레이션 요구사항의 처음 20자를 제목으로 사용) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">추론 재생</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3「시뮬레이션 시작」과 Step5「심층 상호작용」은 실행 중에만 시작 가능하며, 기록 재생은 지원되지 않습니다</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // 현재 선택된 프로젝트 (모달용)
let observer = null
...
const containerStyle = computed(() => {
  ...
  // 실제 필요한 높이 계산: 행 수 * 카드 높이 + (행 수-1) * 간격 + 약간의 하단 간격
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 이전 디바운스 타이머 제거 (새 스크롤 의도가 이전 것을 덮어씀)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 대기 상태가 여전히 실행이 필요한지 확인 (후속 스크롤로 덮어쓰였을 수 있음)
          ...
          // 애니메이션 완료 후 잠금 해제, 대기 중인 상태 변경 확인
          setTimeout(() => {
            isAnimating = false
            
            // 애니메이션 종료 후, 새로운 대기 상태가 있는지 확인
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // 약간의 지연 후 실행, 너무 빠른 전환 방지
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// 라우트 변경 감지, 홈페이지로 돌아올 때 데이터 다시 로드
...
// keep-alive 사용 시, 컴포넌트 활성화될 때 데이터 다시 로드
onActivated(() => {
  loadHistory()
})
...
/* CSS 배경 패턴으로 고정 간격의 정사각형 그리드 생성 */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* 좌상단부터 배치, 높이 변경 시 하단에서만 확장하여 기존 그리드 위치에 영향 없음 */
...
.cards-container {
  ...
  /* min-height는 JS에서 동적으로 계산, 카드 수에 따라 자동 조정 */
}
