        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent와 대화하여 보고서 내용을 심층적으로 이해하기' : '시뮬레이션 개인과 대화하여 그들의 관점을 이해하기' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            ...
            <div class="setup-section">
              ...
              <textarea 
                ...
                class="survey-input"
                placeholder="선택된 대상에게 질문할 내용을 입력하세요..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ surveyResults.length }}건의 응답</span>
            </div>
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('먼저 시뮬레이션 개인을 선택하세요')
  }
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${surveyResults.value.length}건의 응답 수신`)
    } else {
      throw new Error(res.error || '요청 실패')
    }
  } catch (err) {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length}명의 시뮬레이션 개인 로드 완료`)
    }
  } catch (err) {
    addLog(`시뮬레이션 개인 로드 실패: ${err.message}`)
  }
}
...
/* 순서 리스트 번호 수정 - CSS 카운터를 사용하여 여러 ol의 연속 번호 매기기 */
.message-text {
