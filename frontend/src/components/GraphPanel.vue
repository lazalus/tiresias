    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">일부 콘텐츠가 아직 처리 중입니다. 잠시 후 수동으로 그래프를 새로고침해 주세요</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 시뮬레이션 상태에서 비시뮬레이션 상태로 전환, 종료 알림 표시
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// 셀프루프 항목 펼치기/접기 상태 토글
...
const renderGraph = () => {
  ...
  // 각 노드 쌍의 현재 처리 중인 엣지 인덱스 기록
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 이미 처리된 셀프루프 노드
  ...
  tempEdges.forEach(e => {
    ...
    // 엣지 방향이 표준화된 방향과 일치하는지 판단 (소스 UUID < 타겟 UUID)
    ...
    if (totalCount > 1) {
      // 곡률을 균등 분배하여 명확히 구분
      // 곡률 범위는 엣지 수에 따라 증가, 엣지가 많을수록 곡률 범위가 커짐
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // 엣지 방향이 표준화 방향과 반대일 경우 곡률을 반전
      // 이를 통해 모든 엣지가 동일 기준계에서 분포되어 방향 차이로 인한 겹침 방지
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // 원호로 셀프루프 그리기 (sweep-flag=1 시계 방향)
    ...
    // 연결선 방향에 수직인 오프셋, 거리 비율에 따라 계산하여 곡선이 명확히 보이도록 함
    // 엣지가 많을수록 오프셋이 거리에서 차지하는 비율이 커짐
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, 엣지 하나 추가될 때마다 5% 증가
  ...
  const getLinkMidpoint = (d) => {
    ...
    // 2차 베지에 곡선 공식 B(t) = (1-t)^2*P0 + 2(1-t)tP1 + t^2*P2, t=0.5
  ...
  // Link labels background (흰색 배경으로 텍스트를 더 선명하게)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // 위치만 기록, 시뮬레이션 재시작하지 않음 (클릭과 드래그 구분)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // 실제 드래그가 시작되었는지 감지 (이동이 임계값 초과)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // 최초로 실제 드래그 감지 시에만 시뮬레이션 재시작
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // 실제 드래그가 있었을 때만 시뮬레이션을 점진적으로 중지
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // 엣지 라벨 위치 업데이트 (회전 없이 수평 표시로 더 선명하게)
