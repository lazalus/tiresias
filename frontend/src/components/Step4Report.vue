            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (콘텐츠 생성 완료, 전체 섹션은 아직 미완료일 수 있음) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트 후 버튼이 같은 위치에 유지되도록 스크롤 위치 조정
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 전구 아이콘 - 인사이트를 나타냄
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 지구 아이콘 - 파노라마 검색을 나타냄
  },
  'interview_agents': {
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 데이터베이스 아이콘 - 엔티티를 나타냄
  }
}
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // 플랫폼 구분 태그 없음 (매우 오래된 형식), 전체를 답변으로 처리
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // 각 질문-답변 쌍에 대해 독립적인 플랫폼 선택 상태 유지
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 분할 실패: 첫 번째 질문은 전체 답변 반환, 나머지는 빈 값 반환
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 답변(플레이스홀더가 아닌)이 있고 내용이 다름
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  // Expand/Collapse Button (플레이스홀더 텍스트에는 표시하지 않음)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업 완료 시 상단으로 스크롤; 아니면 하단으로 스크롤하여 최신 로그 추적
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Final Answer: 뒤의 내용 찾기 시도 (다양한 형식 지원)
  // 형식1: Final Answer:\n\n내용
  // 형식2: Final Answer: 내용
  ...
  // 내용이 길고 markdown 형식을 포함하면, 사고 과정 제거 후 반환 시도
